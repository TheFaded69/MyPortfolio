...
def pre_cone_right():
    tube = Mesh('precone.stl')
    return tube

def pre_cone():
    reader = vtk.vtkSTLReader()
    reader.SetFileName('./precone.stl')
    reader.Update()
    mesh = Mesh(reader.GetOutput())
    return mesh
...
if __name__ == '__main__':
    ...
    mesh2 = pre_cone()
    mesh2 = mesh.scale(7, 7, 7)
    mesh2 = mesh2.rotate_y(30)
    ...
    result = implicitly_combine(main=mesh,
                                to_subtract_list=[mesh2],
                                to_add_list=[])
    ...
    # cone2_mapper.SetInputData(mesh2)

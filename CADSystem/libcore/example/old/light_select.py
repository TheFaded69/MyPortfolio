...
class App(ViewportMixin):
    ...
    def char(self, caller, event):
        key = self.interactor.GetKeySym()
        if key == 'a':
            if self.actor:
                self.remove_prop(self.actor)

            self.actor = PolyActor(Mesh.box(self.bounds), edge_visibility=True, edge_color='black', opacity=0.5)
            self.add_prop(self.actor)
        if key == 'b':
            if self.actor2:
                self.remove_prop(self.actor2)

            mesh = Mesh.cylinder(center=self.original.center, radius=50.0, height=180.0)
            self.actor2 = PolyActor(Mesh.box(mesh.bounds),  edge_visibility=True, edge_color='black', opacity=0.5)

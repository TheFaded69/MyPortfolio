...
#define PERIOD_MIN_DRYING 86400000                 // время минимальной сушки перед возможным повышением температуры
...
{  
    ...
    {         
        ...
        if ((millis() - time_sensors) > PERIOD_SENSORS)
        {
            ...
            bot.sendMessage("419440205", (String)(millis()/1000) + " " + (String)Temp_meas + " " + (String)allData.firstBME_temp + " " + (String)allData.firstBME_humid + " " + (String)allData.firstBME_press + " " + (String)allData.secondBME_temp + " " + (String)allData.secondBME_humid + " " + (String)allData.secondBME_press + " " + (String)allData.thirdBME_temp + " " + (String)allData.thirdBME_humid + " " + (String)allData.thirdBME_press + " " + (String)water_col_1 + " " + (String)water_col_2 + " " + (String)Power + " " + (String)Relay_run , "");
        ...
        if (((millis() - time_min_drying) > PERIOD_MIN_DRYING) && (water_col_1 < 1))
        {
            time_min_drying = 3600 * 1000 * 100;                                                      // больше не сработает
            //Temp_task = 70;                                                                       // повышаем температуру сушки
            regulator.setpoint = Temp_task;                                                       // сообщаем регулятору температуру, которую он должен поддерживать
        }
...
void Bot_EchoMessages()
{
    ...
    {
        bot.sendMessage(bot.message[i][4], bot.message[i][5], "");                  // препарируем сообщение для выделения команды

        String bot_command = bot.message[i][5];
        String bot_chat_id = bot.message[i][4];

        if (bot_command.startsWith("/"))                                            // проверяем, что команда начинается с символа "/"
        {
            int first_space = bot_command.indexOf(" "); // находим первый побел

            if (first_space != -1)                                                    // выделяем из команды первую часть
            {
                bot_command_body = bot_command;
                //Serial.println(bot_command_body);
